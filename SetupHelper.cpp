...
using RegisterOffIdEstComponentWFn = long (*)(OfflineIdEstCompInfoW*, wchar_t*);
using UnregisterComponentWFn = long (*)(wchar_t*, wchar_t*);
using UnregisterOffIdEstComponentWFn = long (*)(wchar_t*, wchar_t*);
...
constexpr wchar_t kOfflineComponentId[] = L"{33EB9B69-0CC4-4561-8E50-B91E8536B2AD}";
constexpr wchar_t kOfflineComponentName[] = L"My Offline Component";
constexpr wchar_t kOfflineComponentDescription[] = L"My offline identity establishment component";
constexpr wchar_t kOfflineComponentPath[] = L"C:\\Program Files (x86)\\Seclore\\FileSecure\\Desktop Client\\Addon\\OfflineAgent\\OfflineAgent.dll";
constexpr wchar_t kOfflineComponentPath64[] = L"C:\\Program Files\\Seclore\\FileSecure\\Desktop Client\\Addon\\OfflineAgent\\OfflineAgent.dll";
constexpr DWORD kOfflineOptions = OPTION_MASK_INTERACTIVE | OPTION_UNMASK_INTERACTIVE;
...
OfflineIdEstCompInfoW createOfflineComponentInfo()
{
    OfflineIdEstCompInfoW info{};
    info.mlSize = OFFLINE_ID_EST_COMP_INFOW_VER3;
    info.mlOptions = kOfflineOptions;
    info.mOfflineCompConfig.mlSize = OFFLINE_COMP_CONFIGW_VER1_SIZE;

    copyToBuffer(kOfflineComponentId, info.mstrId, std::size(info.mstrId));
    copyToBuffer(kOfflineComponentPath, info.mstrPath, std::size(info.mstrPath));
    copyToBuffer(kOfflineComponentName, info.mstrName, std::size(info.mstrName));
    copyToBuffer(kOfflineComponentDescription, info.mstrDesc, std::size(info.mstrDesc));
    copyToBuffer(kVersion, info.mstrVersion, std::size(info.mstrVersion));
    copyToBuffer(kInstallPath, info.mstrInstallDir, std::size(info.mstrInstallDir));
    copyToBuffer(kOfflineComponentPath64, info.mstrPath64, std::size(info.mstrPath64));
...
int registerComponent(HMODULE module)
{
    ...
    std::wcout << L"Register component result: " << result << L'\n';
    if (error[0] != L'\0')
    {
        std::wcout << L"Register component error: " << error << L'\n';
    }
    ...
    const auto registerOfflineFn =
        getFunction<RegisterOffIdEstComponentWFn>(module, "IRegisterOffIdEstComponentW");
    if (!registerOfflineFn)
    {
        std::cerr << "Unable to locate export: IRegisterOffIdEstComponentW\n";
        return 1;
    }

    OfflineIdEstCompInfoW offlineInfo = createOfflineComponentInfo();
    std::wmemset(error, 0, std::size(error));
    const long offlineResult = registerOfflineFn(&offlineInfo, error);

    std::wcout << L"Register offline component result: " << offlineResult << L'\n';
    if (error[0] != L'\0')
    {
        std::wcout << L"Register offline component error: " << error << L'\n';
    }

    return offlineResult == SETUP_SUCCESS ? 0 : static_cast<int>(offlineResult);
}

int unregisterComponent(HMODULE module)
{
    const auto unregisterOfflineFn =
        getFunction<UnregisterOffIdEstComponentWFn>(module, "IUnregisterOffIdEstComponentW");
    if (!unregisterOfflineFn)
    {
        std::cerr << "Unable to locate export: IUnregisterOffIdEstComponentW\n";
        return 1;
    }
    ...
    wchar_t error[ERROR_MSG_MAX_LEN + 1]{};
    wchar_t offlineComponentId[MAX_OFFLINE_COMP_ID_LEN + 1]{};
    copyToBuffer(kOfflineComponentId, offlineComponentId, std::size(offlineComponentId));

    const long offlineResult = unregisterOfflineFn(offlineComponentId, error);

    std::wcout << L"Unregister offline component result: " << offlineResult << L'\n';
    if (error[0] != L'\0')
    {
        std::wcout << L"Unregister offline component error: " << error << L'\n';
    }

    if (offlineResult != SETUP_SUCCESS)
    {
        return static_cast<int>(offlineResult);
    }
    ...
    std::wcout << L"Unregister component result: " << result << L'\n';
    if (error[0] != L'\0')
    {
        std::wcout << L"Unregister component error: " << error << L'\n';
    }
